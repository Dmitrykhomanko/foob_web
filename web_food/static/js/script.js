...
sendMailButton.addEventListener('click', function() {
    ...
    extraLi.forEach(function(item) {
        ingredientNames.push(item.textContent);
    });

    // Convert the array of ingredient names to a URL query string
    let queryString = ingredientNames.join(' ');

    // Create the URL with the query string
    let relativeUrl = `/send-email/?ingredients=${queryString}`;
